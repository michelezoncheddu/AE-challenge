...
private:
  ...
  template <bool Upper> struct Hull : private std::vector<StoredPoint> {
    const SY epsilon;
...
template <typename X, typename Y> class OptimalPiecewiseLinearModel<X, Y>::CanonicalSegment {
  ...
  bool one_point() const {
    return rectangle[0].x == rectangle[2].x && rectangle[0].y == rectangle[2].y && rectangle[1].x == rectangle[3].x &&
...
template <typename Fin, typename Fout> size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out) {
  if (n == 0)
    return 0;
...
template <typename Fin, typename Fout> size_t make_segmentation_par(size_t n, size_t epsilon, Fin in, Fout out) {
...
#pragma omp parallel for reduction(+ : c) num_threads(parallelism) schedule(guided)
...
template <typename RandomIt> auto make_segmentation(RandomIt first, RandomIt last, size_t epsilon) {
...
struct Segment {
  uint64_t key;      ///< The first key that the segment indexes.
  double slope;      ///< The slope of the segment.
  int32_t intercept; ///< The intercept of the segment.

  Segment() = default;

  Segment(uint64_t key, double slope, int32_t intercept) : key(key), slope(slope), intercept(intercept) {}

  friend bool operator<(const uint64_t k, const Segment &s) { return k < s.key; }
...
std::vector<Segment> make_pgm_segments(const std::vector<uint64_t> &data, size_t epsilon) {
  std::vector<Segment> out;
  out.reserve(data.size() / (epsilon * epsilon));

  auto in_fun = [&](auto i) {
    auto x = data[i];
    if (i > 0 && i + 1u < data.size() && x == data[i - 1] && x != data[i + 1] && x + 1 != data[i + 1])
      return std::pair<uint64_t, size_t>(x + 1, i); // technicality to predict the correct pos when keys are repeated
    return std::pair<uint64_t, size_t>(x, i);
  };

  auto out_fun = [&](auto cs) {
    auto [cs_slope, cs_intercept] = cs.get_floating_point_segment(cs.get_first_x());
    out.emplace_back(cs.get_first_x(), cs_slope, std::round(cs_intercept));
  };
